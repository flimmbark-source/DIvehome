...
export default function CraftMesh({
  ...
  const combatPresentation = Boolean(flashRef)
  const baseScale = APPARATUS_CONFIG.CRAFT_MODEL_BASE_SCALE ?? 0.84
  const targetScale = APPARATUS_CONFIG.CRAFT_VISUAL_SCALE ?? baseScale
  const combatOffset = combatPresentation
    ? [0, (APPARATUS_CONFIG.CRAFT_CURSOR_Y_OFFSET ?? 0) / baseScale, 0]
    : [0, 0, 0]
  const presentationScale = combatPresentation ? targetScale / baseScale : 1
